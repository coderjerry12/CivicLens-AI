...
type QuizState = 'idle' | 'playing' | 'finished' | 'generating';
...
export default function QuizPage() {
  ...
  const startQuiz = useCallback(async () => {
    ...
    // Try generating questions with Gemini AI
    try {
      const { generateQuizQuestions } = await import('@/services/quizGeneratorService');
      const aiQuestions = await generateQuizQuestions(selectedCategory || undefined);
      if (aiQuestions && aiQuestions.length > 0) {
        setShuffledQuestions(aiQuestions);
        setIsAIGenerated(true);
        setState('playing');
        return;
      }
    } catch (err) {
      console.warn('[Quiz] AI generation failed, using static questions:', err);
    }
        ...
        <Card className="text-center py-12">
          <div className="flex flex-col items-center gap-4">
            <div className="relative">
              <Brain className="h-12 w-12 text-secondary-500 animate-pulse" />
              <Zap className="h-5 w-5 text-accent-500 absolute -top-1 -right-1 animate-bounce" />
            </div>
            <h2 className="text-lg font-bold text-neutral-800 dark:text-white">
              Generating Questions with AI...
            </h2>
            <p className="text-sm text-neutral-500 dark:text-neutral-400 max-w-xs">
              Gemini AI is creating unique questions just for you. This takes a few seconds.
            </p>
            <div className="h-1.5 w-48 rounded-full bg-neutral-200 dark:bg-neutral-700 overflow-hidden">
              <div className="h-full w-full rounded-full bg-gradient-to-r from-primary-500 to-secondary-500 animate-pulse" />
            </div>
          </div>
        </Card>
      )}
            ...
            <CardContent className="mt-4">
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-6 max-w-md mx-auto">
                  ...
                  Questions are generated by AI — unique every time! Earn +20 points for a perfect score.
                </p>
