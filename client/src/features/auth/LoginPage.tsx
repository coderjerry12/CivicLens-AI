...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSignedIn(true);
      // AuthLayout will redirect once auth state updates
    } catch (err) {
  ...
  return (
    <>
      {signedIn && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-background rounded-[20px]">
          <div className="flex h-12 w-12 items-center justify-center rounded-[14px] bg-primary-600 mb-3 animate-pulse">
            <Sparkles className="h-6 w-6 text-white" />
          </div>
          <p className="text-sm font-medium text-text-primary">Signing you in...</p>
        </div>
      )}
      <Card className="w-full animate-slide-up relative overflow-hidden">
      <CardContent>
